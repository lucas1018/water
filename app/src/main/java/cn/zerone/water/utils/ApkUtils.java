...
public class ApkUtils {
    ...
    public static Intent getApkInStallIntent(Context context, File apkFile) {
        ...
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
      //  FileProvider.getUriForFile(context.getApplicationContext(), "com.drcnet.fileprovider", file);
        if (Build.VERSION.SDK_INT > 24) {
            Uri uri = FileProvider.getUriForFile(context,  "update.provider", apkFile);
    ...
    private static Uri getApkUri(File apkFile) {
        //Log.d(Constant.TAG, apkFile.toString());
    ...
    //    Log.d(Constant.TAG, uri.toString());
        return uri;
    }
